                    ...
                    <form action="{{ route('jurnal.balik.create') }}" method="get" class="row" id="form-submit">
                        <div class="col-12">
                            ...
                            <table class="table table-sm" id="table-debit">
                                ...
                                <tr>
                                    <td><input type="checkbox" onchange="check(this)" name="id[]"></td>
                                    <td><input type="date" style="width: 100%" name="tujuan" value="{{ request('tujuan') }}"></td>
                                    ...
                                    <td><input type="date" style="width: 100%" name="created_at" value="{{ request('created_at') ?? date('Y-m-d') }}"></td>
                                </tr>
                            </table>
                        </div>
            ...
            <div class="col-6 mt-3">
                <div class="card p-2">
                    <form action="{{ route('jurnal.balik.store') }}" method="post">
                            ...
                            <table class="table table-sm" style="font-size: .7rem; white-space:nowrap">
                                ...
                                <tbody>
                                    @foreach ($data as $idx => $item)
                                        <input type="hidden" value="{{ $item->nomor }}" name="jurnal[{{ $idx }}][nomor]">
                                        ...
                                        <input type="hidden" value="{{ request('created_at')}}" name="jurnal[{{ $idx }}][created_at]">
                                        <tr>
                                            <td>{{ $item->nomor }}</td>
                                            @if ($item->order)
                                                <td>{{ $item->order->job }}-{{ sprintf('%02d',$item->order->no_job) }}</td>
                        ...
                        <button type="submit" class="btn btn-success btn-sm mt-3">Simpan Jurnal Balik</button>
                    </form>
                </div>
            </div>
            @endif
