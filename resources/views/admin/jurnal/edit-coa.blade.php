            ...
            <div class="row mb-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="date" name="created_at" class="form-control" readonly
                        value="{{ date('Y-m-d', strtotime($data[0]->created_at)) }}">
                </div>
            </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <select class="form-select select2" name="jurnal[{{ $j->id }}][coa_id]"
                                    style="width: 100%" data-id="{{ $j->id }}" data-original="{{ $j->coa_id }}">
                            ...
                            <td>
                                {{ $j->order_trucking?->kendaraan
                                    ? $j->order_trucking->kendaraan->nopol .
                                        ' | ' .
                                        $j->order_trucking->kendaraan->milik .
                                        ' | ' .
                                        ($j->order_trucking?->order?->trucking ?? '-')
                                    : ($j->nopol ?? '-') . ' | -' }}
                            </td>
                            <td>{{ $j->nama }}</td>
    ...
    <script>
        ...
        $('#btn-save').click(function(e) {
            if (total_debit != total_credit) {
                alert('Jurnal Tidak Balance debit = ' + total_debit + ' & credit = ' + total_credit +
                    ' ! Harap check lagi')
            } else {
