        ...
        <div class="row">
            @if ($tipe == 'xpdc')
                <div class="col-12 mb-3">
                    ...
                    <select class="form-control select2" id="job-{{ $jurnal->id }}" name="order_id"
                        style="font-size:.9rem !important">
                        <option value=""></option>
                        @foreach ($orders as $item)
                            <option {{ $jurnal->order_id == $item->id ? 'selected' : '' }} value="{{ $item->id }}">
                                {{ $item->job }}-{{ sprintf('%02d', $item->no_job) }} / {{ $item->seal }} /
                                {{ $item->invoice }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            @if ($tipe == 'trucking')
                <div class="col-12 mb-3">
                    ...
                    <select class="form-control select2" id="job-{{ $jurnal->id }}" name="order_trucking_id"
                        style="font-size:.9rem !important">
                        <option value=""></option>
                        @foreach ($orders as $item)
            ...
            <div class="col-12 mb-3">
                ...
                <input class="form-control" onclick="this.select()" type="text" name="invoice_external"
                    id="invoice_external-{{ $jurnal->id }}" value="{{ $jurnal->invoice_external }}">
            </div>
            <div class="col-12 mb-3">
