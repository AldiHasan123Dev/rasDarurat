...
@section('content')
                ...
                <div class="d-flex flex-wrap" style="gap:10px;">
                    @if (Auth::user()->role_id == 1 || Auth::id() == 5)
                        <button class="py-2 px-3 btn btn-sm btn-success" type="button" data-bs-toggle="modal"
                            data-bs-target="#modal-export">Export Excel</button>
                    @endif
                    @if (!request('filter-order') && is_null($marketing))
                        <button class="py-2 px-3 btn btn-sm btn-success" data-bs-toggle="offcanvas"
                            data-bs-target="#offcanvasOrder" aria-controls="offcanvasOrder">Tambah Order</button>
                        <button class="py-2 px-3 btn btn-sm btn-success" type="button" data-bs-toggle="modal"
                            data-bs-target="#modal-export-malindo">JOB Malindo</button>
                        <button class="py-2 px-3 btn btn-sm btn-success" type="button" data-bs-toggle="modal"
                            data-bs-target="#modal-export-cheiljedang">JOB PT. CJ. CHEILJEDANG</button>
                        <button class="py-2 px-3 btn btn-sm btn-success" type="button" data-bs-toggle="modal"
                    ...
                    @if (is_null($marketing))
                        <button type="button" onclick="modalEditOrder()" id="order-edit"
                            class="py-2 px-3 btn btn-sm btn-primary">Edit Order</button>
                    @endif
                    @if (is_null($marketing))
                        <button onclick="printPackingList()" id="packing-list"
                            class="py-2 px-3 btn btn-sm btn-warning">Packing List</button>
                        <button onclick="printPackingListKubikasi()" id="packing-list-kubikasi"
                            class="py-2 px-3 btn btn-sm btn-warning">Packing List Kubikasi</button>
                        <button onclick="modalPindahKapal()" id="btn-pindah-kapal"
                            class="py-2 px-3 btn btn-sm btn-info">Pindah Kapal</button>
                        <form action="" id="copy-order" method="post" enctype="multipart/form-data">
                            @csrf
                            <button class="py-2 px-3 btn btn-sm btn-secondary" type="button" id="copy-order-btn"
                                onclick="return confirm('Are you sure?')">Copy Order</button>
                        </form>
                        <form action="" id="delete-order" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('DELETE')
                            <button class="py-2 px-3 btn btn-sm btn-danger" type="submit"
                                onclick="return confirm('Are you sure?')">Hapus Order</button>
                        </form>
                        <form action="" id="tarik-ba" method="post">
                            @csrf
                            @method('PUT')
                            @if (is_null($marketing))
                                <button class="py-2 px-3 btn btn-sm btn-warning" name="ba" value="2"
                                    type="submit" onclick="return confirm('Are you sure?')">Tarik BA kembali</button>
                            @endif
                        </form>
                        <button data-bs-toggle="modal" data-bs-target="#tagihan" class="btn btn-sm btn-success"
                            id="btn-tagihan">Tambah Tagihan</button>
                    @endif
                    @if (Auth::user()->role_id == 1)
                        <button class="py-2 px-3 btn btn-sm btn-light text-dark border border-dark"
                            id="btn-lock">-</button>
                    @endif
                ...
                <div class="card">
                    <div class="card-header" style="gap:10px" id="bttb-info">
                        ...
                        <div class="p-2 d-flex" style="gap:10px" id="bttb-info">
                            @if (is_null($marketing))
                                <button class="py-2 px-3 btn btn-sm btn-success" type="button" data-bs-toggle="offcanvas"
                                    data-bs-target="#offcanvasBTTBCreate"><i class="fas fa-plus"></i> Tambah BTTB</button>
                                <button onclick="printBttb()" class="py-2 px-3 btn btn-sm btn-secondary"
                                    style="font-size: .7rem" id="bttb-print"><i class="fas fa-print"></i> Print
                                    BTTB</button>
                                <button onclick="printBttb(true)" class="py-2 px-3 btn btn-sm btn-secondary"
                                    style="font-size: .7rem" id="bttb-print"><i class="fas fa-print"></i> Print BTTB Inc
                                    Berat</button>
                                <button onclick="printBttbKubikasi()" class="py-2 px-3 btn btn-sm btn-secondary"
                                    style="font-size: .7rem" id="bttb-kubikasi-print"><i class="fas fa-print"></i> Print
                                    BTTB Kubikasi</button>
                                <button onclick="printBttbKubikasi(true)" class="py-2 px-3 btn btn-sm btn-secondary"
                                    style="font-size: .7rem" id="bttb-kubikasi-print"><i class="fas fa-print"></i> Print
                                    BTTB Kubikasi Inc Berat</button>
                                <a class="py-2 px-3 btn btn-sm btn-info" style="font-size: .7rem" id="edit-bttb"><i
                                        class="fas fa-pencil"></i> Edit</a>
                                <button class="py-2 px-3 btn btn-sm btn-danger" style="font-size: .7rem"
                                    id="delete-bttb"><i class="fas fa-trash"></i> Hapus</button>
                            @endif
                        </div>
                    </div>
                    <div class="card-body">
        ...
        <div class="offcanvas-body">
            <form action="{{ route('order.store') }}" method="post" id="create">
                ...
                <div class="col-12 mb-2 px-1">
                    <button type="button" id="add-order"
                        class="btn btn-success btn-sm">{{ empty($order) ? 'Tambah' : 'Update' }} Data</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="offcanvas offcanvas-bottom" tabindex="-2" id="offcanvasBTTBCreate" aria-labelledby="offcanvasBTTBLabel"
        style="height: 700px">
        <div class="offcanvas-header">
            <h5 class="offcanvas-title" id="offcanvasBTTBLabel">Form BTTB</h5>
            <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        ...
        <div class="offcanvas-body">
            <b>*Harap Pastikan No Gudang, Nama Barang, Jumlah, Satuan & Pengirim Terisi. Lalu pada inputan pengirim, harap
                pilih data yang sudah disediakan!</b>
            <form id="form-bttb-create">
                ...
                <table class="w-100 table-bordered" style="font-size: .7rem; table-layout:auto">
                    ...
                    <tbody>
                        @for ($i = 0; $i < 12; $i++)
                            <tr>
                                <td><input type="text" style="width: 100px" name="no_gudang-{{ $i }}"
                                        id="no_gudang-{{ $i }}"></td>
                                <td><input name="barang_id-{{ $i }}" id="barang_id-{{ $i }}"
                                        class="barang" style="width: 200px" /></td>
                                <td><input type="number" style="width: 70px" name="qty-{{ $i }}"
                                        id="qty-{{ $i }}" /></td>
                                <td><input name="satuan_id-{{ $i }}" id="satuan_id-{{ $i }}"
                                        class="satuan" style="width: 100px" /></td>
                                <td><input type="number" step="any" onkeyup="hitungVolCreate({{ $i }})"
                                        style="width: 70px" name="p-{{ $i }}" id="p-{{ $i }}">
                                </td>
                                <td><input type="number" step="any" onkeyup="hitungVolCreate({{ $i }})"
                                        style="width: 70px" name="l-{{ $i }}" id="l-{{ $i }}">
                                </td>
                                <td><input type="number" step="any" onkeyup="hitungVolCreate({{ $i }})"
                                        style="width: 70px" name="t-{{ $i }}" id="t-{{ $i }}">
                                </td>
                                <td><input type="number" style="width: 70px" name="vol-{{ $i }}"
                                        id="vol-{{ $i }}"></td>
                                <td><input type="number" style="width: 70px" name="berat-{{ $i }}"
                                        id="berat-{{ $i }}"></td>
                                <td><input type="date" style="width: 100px" name="tgl_masuk-{{ $i }}"
                                        id="tgl_masuk-{{ $i }}"></td>
                                <td><input name="pengirim_id-{{ $i }}" id="pengirim_id-{{ $i }}"
                                        class="pengirim" style="width: 100px" /></td>
                                <td><input type="text" name="keterangan-{{ $i }}"
                                        id="keterangan-{{ $i }}"></td>
                            </tr>
                        @endfor
                    </tbody>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-success"
                        onclick="return confirm('Are you sure?')">Simpan</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="modal fade" id="modal-export-malindo" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
     ...
     <div class="modal fade" id="modal-export-logisted" tabindex="-1" aria-labelledby="exampleModalLabel"
        ...
        <form action="{{ route('order.export.sinar-balado') }}" method="POST" class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Export Data Sumber Balado</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-2">
    ...
    <div class="modal fade" id="modal-export-fortuna" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
    ...
    <div class="modal fade" id="modal-export-cheiljedang" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
    ...
    <script>
        ...
        $('#add-order').click(function(e) {
            $.ajax({
                ...
                url: "{{ route('api.customer.getCustomer') }}",
                data: {
                    nama: [$('#pengirim_id').val(), $('#penerima_id').val()]
                },
                success: function(response) {
                    if (response == 0) {
                        alert(
                            'Pengirim atau Penerima tidak ditemukan di data Customer! silahkan cek data lagi')
                    } else {
                        $('#create').submit();
                    }
                }
        ...
        const printPackingList = () => {
            ...
            let params_ =
                `scrollbars=no,resizable=no,status=no,location=no,toolbar=no,menubar=no,width=800,height=500,left=100,top=100`;
