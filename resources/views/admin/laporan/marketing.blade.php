                            ...
                            <tbody>
                                ...
                                @foreach ($data as $idx => $item)
                                    <tr>
                                        ...
                                        @for ($i = 1; $i <=24; $i++)
                                            @if ($i%2==0)
                                                <th class="text-center">{{ $item->laporanMarketing20Fit($month) }}</th>
                                                @php
                                                    $fit20 += $item->laporanMarketing20Fit($month);
                                                    $sub[$i] = ($sub[$i]??0) + $item->laporanMarketing20Fit($month);
                                                    $month++;
                                                @endphp
                                            @else
                                                <th class="text-center">{{ $item->laporanMarketing40Fit($month) }}</th>
                                                @php
                                                    $fit40 += $item->laporanMarketing40Fit($month);
                                                    $sub[$i] = ($sub[$i]??0) + $item->laporanMarketing40Fit($month);
                                                @endphp
                                            @endif
                                        @endfor
