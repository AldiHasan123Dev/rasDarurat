...
@section('content')
                    ...
                    <div class="card">
                        <div class="card-header p-2 d-flex justify-content-between" style="gap:10px">
                            ...
                            <div class="d-flex gap-3">
                                ...
                                @if ($role != 'cs')
                                    <button type="button" class="btn btn-success" onclick="addDraf()">Buat Draf
                                        Jurnal</button>
                                @endif
                            </div>
                            ...
                            <form action="{{ route('jasakirim.sync.data') }}" method="post">
                                ...
                                <div class="d-flex gap-1">
                                    @if ($role != 'cs')
                                        ...
                                        <button class="btn btn-sm btn-primary" onclick="merge()"
                                            type="button">Merge</button>
                                    @endif
                                    @if ($role == 'cs')
                                        <button class="btn btn-sm btn-info" type="button" id="unmerge">Unmerge</button>
                                    @endif
                                    <button class="btn btn-sm btn-success" onclick="loadListKirimDokumen()"
                                        type="button">Tambah Item Resi</button>
                                                    ...
                                                    <div class="d-flex gap-3">
                                                        @if ($role == 'jurnal')
                                                            @if (!$item->first()->jurnal)
                                                                <a href="{{ route('jasakirim.draf.jurnal', ['invoice' => $item->first()->invoice]) }}"
                                                                    class="btn btn-success py-0 px-5"
                                                                    style="height: 20px; font-size:.7rem">Buat Jurnal</a>
                                                            @else
                                                                <a href="{{ route('jurnal.edit', ['jurnal' => $item->first()->jurnal]) }}"
                                                                    class="btn btn-warning py-0 px-5"
                                                                    style="height: 20px; font-size:.7rem">Lihat Jurnal</a>
                                                                @if (Auth::user()->role_id == 1)
                                                                    <form action="{{ route('jasakirim.sync.jurnal') }}"
                                                                        method="post">
                                                                        @csrf
                                                                        <input type="hidden" name="invoice"
                                                                            value="{{ $item->first()->invoice }}">
                                                                        <button class="btn btn-success py-0 px-5"
                                                                            style="height: 20px; font-size:.7rem"
                                                                            type="submit">Sinkronisasi</button>
                                                                    </form>
                                                                @endif
                                                            @endif
                                                        @endif
                                                        <button class="btn btn-info py-0 px-5"
                                                            style="height: 20px; font-size:.7rem" type="button"
                                                            data-bs-toggle="modal"
                                                            data-bs-target="#listresi-{{ $loop->iteration }}">Detail</button>
                                                    </div>
                                                    <div class="modal fade" id="listresi-{{ $loop->iteration }}"
                                                        tabindex="-1"
                                                        aria-labelledby="listresi-{{ $loop->iteration }}Label"
                                                        aria-hidden="true">
                                                        <div class="modal-dialog modal-xl">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title"
                                                                        id="listresi-{{ $loop->iteration }}Label">List
                                                                        Draf {{ $item->first()->invoice }}</h5>
                                                                ...
                                                                <div class="modal-body">
                                                                    <div class="table-responsive">
                                                                        <table class="table table-sm table table-list"
                                                                            ...
                                                                            <tbody>
                                                                                @foreach ($item as $rs)
                                                                                    <tr>
                                                                                        ...
                                                                                        <td>{{ $rs->agen->lokasi->nama ?? '-' }}
                                                                                        </td>
                                                                                        <td>{{ $rs->order_name() }}</td>
                                                                                        <td>{{ $rs->barcode }}</td>
                                                                                        <td>{{ date('d/m/y', strtotime($rs->tgl_kirim)) }}
                                                                                        </td>
                                                                                        <td>{{ date('d/m/y', strtotime($rs->tgl_terima)) }}
                                                                                        </td>
                                                                                        <td>{{ number_format($rs->nominal) }}
                                                                                        </td>
                                                                                    </tr>
                                                                                @endforeach
                                                                            </tbody>
    ...
    <div class="offcanvas offcanvas-start" tabindex="-2" id="offcanvasJasaKirim"
        aria-labelledby="offcanvasJasaKirimLabel">
                ...
                <div class="modal-body">
                    ...
                    <div class="d-flex gap-2">
                        <input type="text" name="nama" id="nama" class="form-control"
                            placeholder="Tulis keterangan" style="width: 80%">
                        <button class="py-1 px-3 bg-success text-white" type="button" style="width: 20%"
                            onclick="addKirimDokumen()">Simpan</button>
                    </div>
                </div>
                <div class="modal-footer">
...
@section('script')
    ...
    <script>
        ...
        table1.on('click', 'tbody tr', function(e) {
            ...
            $('#nominal-selected').html('Rp. ' + nominal.toLocaleString('id-ID') + '(' + table1.rows('.selected')
                .data().length + ')');
        });
        ...
        function loadListKirimDokumen() {
            if (table1.rows('.selected').data().length == 1) {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ url('api/kirim-dokumen') }}" + '?jasa_kirim_id=' + table1.rows('.selected').data()[0]
                        .id,
                    success: function(response) {
                        let html = '';
