...
@section('content')
    <div class="container">
        <div class="row">
            <div class="col-12">
                <div class="card p-3">
                    <form action="{{ route('role.update',$role) }}" method="post" class="row">
                        @csrf
                        @method('PUT')
                        <div class="col-12">
                            <div class="mb-3">
                                <label for="name">Role Name</label>
                                <input type="text" name="name" id="name" class="form-control w-50" value="{{ $role->name }}">
                            </div>
                            <div class="mb-2">
                                <hr class="py-0">
                                <ul class="parent">
                                    ...
                                    <li style="float: left">
                                        ...
                                        <ul>
                                            ...
                                            <li>
                                                <input {{ $role->access->where('sub_menu_id',$item->id)->first() ? 'checked' : '' }} type="checkbox" name="sub_menu_id[]" id="child-{{ $item->id }}" value="{{ $item->id }}" class="child-checkbox" />
